...
import { InputField, MonthsPeriod, SelectField, AgileMethodologySelect, SelectOrEnterField, TextAreaField, SaveButton } from 'foundation/FormFields/client/components';
...
import { getAgileValidationSchema } from "foundation/Application/client/Experience/validationSchemas";
import FormTracking from 'foundation/Analytics/client/components/AdobeAnalytics/formTracking';
import * as uiActions from "./actions";
import '../Experience/experience.scss';

class AgileExperienceEdit extends React.Component {
  state = {
    isFormStart: false,
  };
  ...
  renderForm = (formikProps) => {
    ...
    const agileMethodologiesOptions = sitecoreListsSettings ? sitecoreListsSettings.AgileMethodologies.map(formatOption) : [];
    ...
    return (
      <Form className="edit-mode experience" onFocus={this.handleFocus}>
        ...
        <div className="form-row">
          <div className="col-md">
            <AgileMethodologySelect
              enumFieldName="agileMethodologyEnum"
              otherFieldName="agileMethodologyOther"
              label={<Text field={fields.EditModeApproach}/>}
              helpText={fields.AgileMethodologyHelpText}
              placeholder={i18n.t('cert-app.Common.SelectPlaceholder')}
              options={agileMethodologiesOptions}
              disabled={saving}
              required
            />
          </div>
  ...
  render() {
    ...
    return (
      <Formik
        ...
        validateOnChange
        validationSchema={getAgileValidationSchema(fields)}
      >
        {this.renderForm}
      </Formik>
    )
